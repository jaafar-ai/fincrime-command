...
window.TICKER_ITEMS = [
  ...
  "Trusted media watch: Reuters and wire reporting on financial crime",
  "USD/IQD watch: official CBI rate vs parallel market"
];
let headlineIndex=0;
setInterval(()=>{
  if(!window.TICKER_ITEMS.length)return;
  headlineIndex=(headlineIndex+1)%window.TICKER_ITEMS.length;
  const el=document.getElementById("tickerText");
  if(el)el.textContent=window.TICKER_ITEMS[headlineIndex];
},5000);
